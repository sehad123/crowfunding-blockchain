...
const Form = () => {
  ...
  const startCampaign = async (e) => {
    ...
    if (form.campaignTitle === "") {
      ...
      //   const contract = new ethers.Contract(process.env.NEXT_PUBLIC_ADDRESS, CampaignFactory.abi, signer);
      const contract = new ethers.Contract("0x31d4C6526A2C5b41AC41eCb0ab05Fd4aFaD86F5C", CampaignFactory.abi, signer);
      //   NEXT_PUBLIC_ADDRESS = "0x31d4C6526A2C5b41AC41eCb0ab05Fd4aFaD86F5C";

      const CampaignAmount = ethers.utils.parseEther(form.requiredAmount);

      const campaignData = await contract.createCampaign(form.campaignTitle, CampaignAmount, imageUrl, form.category, storyUrl);
