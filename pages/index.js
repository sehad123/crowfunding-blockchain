...
export default function Index({ AllData, HealthData, EducationData, AnimalData }) {
  const NEXT_PUBLIC_RPC_URL = "https://polygon-mumbai.infura.io/v3/ade15bd60cc849ed84fb17c15cd7476d";
  const NEXT_PUBLIC_PRIVATE_KEY = "f572e6ef096d2fa44e26e8abbce15580665773e870bd4cd80f94c77156bee462";
  const NEXT_PUBLIC_ADDRESS = "0x31d4C6526A2C5b41AC41eCb0ab05Fd4aFaD86F5C";
      ...
      <CardsWrapper>
        ...
        {filter.map((e) => {
          return (
            <Card key={e.title}>
              <CardImg>
                <Image alt="Crowdfunding dapp" layout="fill" src={"https://crowdfunding.infura-ipfs.io/ipfs/" + e.image} />
                {/* <Image alt="Crowdfunding dapp" layout="fill" src={"https://ipfs.infura.io/ip/fs/" + e.image} /> */}
              </CardImg>
              <Title>{e.title}</Title>
              <CardData>
                <Text>
...
export async function getStaticProps() {
  ...
  const provider = new ethers.providers.JsonRpcProvider(NEXT_PUBLIC_RPC_URL);

  // const contract = new ethers.Contract(process.env.NEXT_PUBLIC_ADDRESS, CampaignFactory.abi, provider);
  const contract = new ethers.Contract(NEXT_PUBLIC_ADDRESS, CampaignFactory.abi, provider);
